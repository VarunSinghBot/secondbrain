...
async function runTestSuite() {
  ...
  recordTest(
    ...
    Boolean(!val3.isValid && val3.error?.includes("exceeds maximum allowed length")),
    performance.now() - start,
  ...
  recordTest(
    ...
    Boolean(!val4.isValid && val4.error?.includes("restricted injection patterns")),
    performance.now() - start,
  ...
  const sampleKnowledgeText =
    ...
    "Key advantage of Quantum computing: Exponential speedup for integer factorization and molecular simulation."
